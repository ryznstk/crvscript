...
# ============================================================
# VoltageOS Peridot Build Script
# ============================================================
...
# -----------------------------
ROM_NAME="VoltageOS"
ROM_URL="https://github.com/VoltageOS/manifest.git"
ROM_BRANCH="17"
...
description() {
    ...
    echo -e "${CYAN}${BOLD}├────────────────────────────────────────────────────────────┤${RESET}"
    echo -e "${CYAN}│${RESET} VoltageOS automated build and release script for         ${CYAN}│${RESET}"
    echo -e "${CYAN}│${RESET} ${BOLD}POCO F6 (peridot)${RESET}.                                       ${CYAN}│${RESET}"
    echo -e "${CYAN}│${RESET}                                                            ${CYAN}│${RESET}"
    echo -e "${CYAN}│${RESET} Source → Sync → Build → Upload → Notify                  ${CYAN}│${RESET}"
    echo -e "${CYAN}│${RESET}                                                            ${CYAN}│${RESET}"
...
notify_build_start() {
    ...
    tg_send "🚀 VoltageOS build started
...
notify_build_failed() {
    ...
    tg_send "❌ VoltageOS build FAILED
...
notify_finished() {
    ...
    tg_send "🏁 VoltageOS build finished
...
section "Initializing VoltageOS"
...
ok "VoltageOS repository initialized"
...
echo "│ Variant    : user                                          │"
echo "│ Build cmd  : brunch peridot user                           │"
echo "╰────────────────────────────────────────────────────────────╯"
echo -e "${RESET}"

section "Generating Signing Keys"

info "Entering vendor/voltage-priv/keys..."
cd vendor/voltage-priv/keys || fail "Failed to enter vendor/voltage-priv/keys"

info "Generating VoltageOS signing keys..."
./keys.sh || fail "VoltageOS key generation failed"

info "Returning to ROM source root..."
croot || fail "Failed to return to ROM source root"

ok "VoltageOS signing keys generated"
info "Build command: brunch peridot user"
...
section "Building VoltageOS"

BUILD_START=$(date +%s)

if brunch peridot user; then
...
info "Searching for the final VoltageOS ZIP (target_files and OTA helper ZIPs excluded)"
...
if [[ ${#ARTIFACTS[@]} -eq 0 ]]; then
    ...
    tg_send "⚠️ VoltageOS build completed
